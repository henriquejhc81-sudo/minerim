...
st.set_page_config(
    ...
    page_icon="⚡",
    layout="wide",
    initial_sidebar_state="collapsed" # Remove a barra lateral
)

# Estilização CSS Premium (Dark Mode Profundo)
st.markdown("""
    <style>
...
@st.cache_data(ttl=120) # Auto-atualiza a cada 2 minutos
def buscar_dados_inteligentes():
    return motor_mestre.varredura_global()

with st.spinner("Autobolt mapeando fractais de 4h e 15m..."):
    dados_mercado = buscar_dados_inteligentes()
    df = pd.DataFrame(dados_mercado)

# Colorização Inteligente
def colorir_veredicto(val):
    if "LIBERADO" in str(val): return 'color: #10b981; font-weight: bold;'
    elif "QUARENTENA" in str(val): return 'color: #ef4444; font-weight: bold;'
    elif "BLOQUEIO MACRO" in str(val): return 'color: #f59e0b; font-weight: bold;'
    return 'color: #64748b;'
...
liberados = len(df[df['Veredicto'].str.contains('LIBERADO')]) if not df.empty else 0
...
for index, row in df.iterrows():
    if "QUARENTENA" in row['Veredicto'] or "BLOQUEIO" in row['Veredicto']:
        st.caption(f"- **{row['Ativo']} ({row['Grupo']}):** Protegido. Motivo: {row['Motivo']}.")
